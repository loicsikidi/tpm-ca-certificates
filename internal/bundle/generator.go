...
func (g *Generator) GenerateWithConcurrencyAndOutput(cfg *config.TPMRootsConfig, workers int, outputPath string) (string, error) {
	return g.GenerateWithMetadata(cfg, workers, outputPath, "", "", TypeRoot)
}
...
// The bundleType parameter determines the type of bundle (root or intermediate).
// If outputPath is empty, no filename is included in the header.
//
// Example:
//
//	gen := bundle.NewGenerator()
//	pemBundle, err := gen.GenerateWithMetadata(cfg, 4, "bundle.pem", "2024-06-15", "a1b2c3d...", bundle.BundleTypeRoot)
//	if err != nil {
//	    log.Fatal(err)
//	}
//	fmt.Println(pemBundle)
func (g *Generator) GenerateWithMetadata(cfg *config.TPMRootsConfig, workers int, outputPath, date, commit string, bundleType BundleType) (string, error) {
	if workers == 0 {
