...
- **No long-lived keys**: Signatures use ephemeral certificates bound to GitHub Actions OIDC tokens
...
### 1. Integrity Verification (Cosign Signature)
...
If you already have the bundle, you can verify them manually:
...
When checksum files are not provided, `tpmtb` follows this strategy:

1. Looks for `checksums.txt` and `checksums.txt.sigstore.json` in the same directory as the bundle
2. If not found, downloads them from the GitHub release matching the bundle's date tag
3. Automatically cleans up downloaded files after verification
...
cosign verify-blob \
  ...
  --certificate-identity-regexp 'https://github.com/loicsikidi/tpm-ca-certificates/.github/workflows/release-bundle.yaml@refs/tags/2025-12-03' \
...
This command:
- Uses the verified checksum as the trust anchor
- Queries GitHub API for SLSA build provenance attestations
- Validates that the bundle was built from the expected Git commit
- Ensures all metadata is consistent (date, commit, repository)

> [!IMPORTANT]
> Both steps are required for complete security. The integrity check (Step 1) must be performed **before** trusting the attestation verification (Step 2), as the checksum serves as the trust anchor for all subsequent verifications.
...
> [!WARNING]
> If integrity verification fails for an official release from the repository, please create [an issue](https://github.com/loicsikidi/tpm-ca-certificates/issues/new) in the repository immediately. This could indicate a supply chain attack.
