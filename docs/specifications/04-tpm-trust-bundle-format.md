...
| alpha   | 2026-05-19 | Loïc Sikidi | Fix typo in intermediate's template |
...
> [!IMPORTANT]
> The bundle type (root or intermediate) is indicated in the global metadata description line. Consuming software can identify the bundle type from this metadata.
...
## and contains a list of verified TPM <BUNDLE_TYPE> Endorsement Certificates.
##
```
...
- A descriptive block explicitly indicating the file was auto-generated and the bundle type:
  - For **root certificates**: `TPM Root Endorsement Certificates`
  - For **intermediate certificates**: `TPM Intermediate Endorsement Certificates`
  ```
  ## This file has been auto-generated by tpmtb (TPM Trust Bundle)
  ## and contains a list of verified TPM <BUNDLE_TYPE> Endorsement Certificates.
  ```
...
The CLI determines the bundle type using the following priority:
1. **Explicit flag**: If `--type` flag is provided (`root` or `intermediate`), use that value
2. **(Implicit) Configuration file name**: Otherwise, infer from the config file name:
   - If the config file is `.tpm-intermediates.yaml` → **intermediate** bundle type
   - Otherwise → **root** bundle type

The corresponding description line will be:
- **Root bundle**: `## and contains a list of verified TPM Root Endorsement Certificates.`
- **Intermediate bundle**: `## and contains a list of verified TPM Intermediate Endorsement Certificates.`
...
- **Bundle type identification**: The description line clearly indicates whether the bundle contains root or intermediate certificates
